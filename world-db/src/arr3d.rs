...
mod tests {
    ...
    fn serialize_and_deserialize_bincode_fake_voxel() {
        let mut arr3d: Arr3d<Option<VoxelId>> = Arr3d::default();
        *arr3d.get_mut(PosU(1,2,5)) = Some(fake_voxel(12));
        *arr3d.get_mut(PosU(15,4,0)) = Some(fake_voxel(58));
        let bin = bincode::serialize(&arr3d).unwrap();
        let de_ser: Arr3d<Option<VoxelId>> = bincode::deserialize(&bin).unwrap();
        assert_eq!(arr3d,de_ser);
    }
}