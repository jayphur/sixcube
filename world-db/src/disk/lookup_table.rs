...
pub const LOOKUP_TABLE_BYTE_LENGTH: usize = {((16*16*16)*64 + 64 + (16*16*16)*16) / 8 + 16 + 16}; //first + 16 => end, second + 16 => length
...
impl LookupTable {
	pub fn end(&self) -> u64{
		self.end + LOOKUP_TABLE_BYTE_LENGTH as u64
	}
	pub fn start(&self, u: usize) -> u64{
		self.start[u] + LOOKUP_TABLE_BYTE_LENGTH as u64
	}

	///w\\o padding, use `length_of_with_padding` for w\\ padding
	pub fn length_of(&self, u: usize) -> u64{
	...
	pub fn fit(&mut self, index: usize, amount: u64) -> u64{
	...
	pub fn set_padding(&mut self, index:usize, amount: u64){
...
mod tests {
	...
	fn lookup_table_round_trip() {
		...
		*table.start.get_mut(1).unwrap() = 342;
		...
		assert_eq!(bytes.len(), LOOKUP_TABLE_BYTE_LENGTH);
		assert_eq!(LookupTable::from_bytes(&bytes).unwrap(), table); // Round Trip
	...
	fn lookup_table_byte_size(){
		...
		assert_eq!(table.to_bytes().len(), LOOKUP_TABLE_BYTE_LENGTH); // LOOKUP_TABLE_SIZE is accurate
	}
	...
	fn lookup_table_padding() {
		...
		table.fit(0, 100);
		for x in 1..16*16*16 {
			assert_eq!(table.start(x), 100 + LOOKUP_TABLE_BYTE_LENGTH as u64);
		}
		table.fit(0, 200);
		...
		for x in 1..16*16*16 {
			assert_eq!(table.start(x), 200 + LOOKUP_TABLE_BYTE_LENGTH as u64);
		}
		table.fit(0, 250);
		assert_eq!(table.length_of(0), 250);
		for x in 1..16*16*16 {
			assert_eq!(table.start(x), 250 + LOOKUP_TABLE_BYTE_LENGTH as u64);
		}
	}
	...
	fn lookup_table_fit() {
		...
		for x in 0..20 {
			assert_eq!(table.start(x), x as u64 *10 + LOOKUP_TABLE_BYTE_LENGTH as u64);
		}
		table.fit(10, 25);
		for x in 0..10 {
			assert_eq!(table.length_of(x), 10);
		...
		for x in 11..20 {
			assert_eq!(table.length_of(x), 10);
			assert_eq!(table.start(x), x as u64 *10 + 15 + LOOKUP_TABLE_BYTE_LENGTH as u64);
		}
